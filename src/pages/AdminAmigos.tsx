...
export default function AdminAmigos() {
  ...
  const [registros, setRegistros] = useState<Amigo[]>([]);
  ...
  useEffect(() => {
    ...
    const fetchAll = async () => {
      try {
        const res = await fetch(`${API_BASE_URL}/api/admin/mil-amigos-home`, { headers: authHeader });
    ...
    const fetchOne = async () => {
      ...
      try {
        const res = await fetch(`${API_BASE_URL}/api/admin/mil-amigos-home/${id}`, { headers: authHeader });
        if (!res.ok) throw new Error('Registro não encontrado');
        const data: Amigo = await res.json();
        setTitle(data.title);
        const clean = data.content.replace(/style="[^"]*color:[^;"]*;?[^"]*"/g, '');
        setContent(clean);
        if (data.image_path) {
          const img = `${API_BASE_URL}${data.image_path}`;
          setPreview(img);
          setOldImagePath(data.image_path); // já é relativo
        }
      } catch (e) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const url = id ? `${API_BASE_URL}/api/admin/mil-amigos-home/${id}` : `${API_BASE_URL}/api/admin/mil-amigos-home`;
      ...
      const list = await fetch(`${API_BASE_URL}/api/admin/mil-amigos-home`, { headers: authHeader });
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-900 to-green-600 text-white p-6 flex justify-center">
      <div className="w-full max-w-4xl">
        <h1 className="text-4xl font-semibold mb-8">{id ? 'Editar Mil Amigos' : 'Novo Mil Amigos'}</h1>
        ...
        <div className="grid gap-6 mb-16">
          {registros.length > 0 ? registros.map((r) => (
            <div key={r.id} className="bg-white text-gray-800 p-4 rounded-lg shadow-md flex justify-between items-center">
              ...
              <Link to={`/admin/amigos/${r.id}`} className="text-blue-600 hover:underline">✏️ Editar</Link>
            </div>
          )) : (
